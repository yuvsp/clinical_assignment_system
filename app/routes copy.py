...
def assign_instructor(student_id):
            ...
            if assigned_count < instructor.max_students_per_day:
                if instructor.area_of_expertise in preferred_fields:
                    if day not in student_assigned_days:
                        relevant_instructors.append((instructor, day, preferred_fields.index(instructor.area_of_expertise)))
                    else:
    ...
    # Sort relevant instructors by preference
    relevant_instructors.sort(key=lambda x: x[2])
...
def relevant_instructors(student_id):
    ...
    for instructor in all_instructors:
        ...
        for day in available_days:
            ...
            if assigned_count < instructor.max_students_per_day:
                if instructor.area_of_expertise in preferred_fields:
                    if day not in student_assigned_days:
                        relevant_instructors.append({'name': instructor.name, 'area_of_expertise': instructor.area_of_expertise, 'day': day, 'preference': preferred_fields.index(instructor.area_of_expertise)})
                    else:
    ...
    # Sort relevant instructors by preference
    relevant_instructors.sort(key=lambda x: x['preference'])
